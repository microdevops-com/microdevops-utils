...
def load_yaml_config(d, f):
    ...
    logging.info("Loading JSON config {0}/{1}".format(d, f))
...
if __name__ == "__main__":
    ...
    with open_history_message_file(HISTORY_DIR, "{0}_{1}.{2}".format(HISTORY_MESSAGE_IN_PREFIX, key_combo, HISTORY_MESSAGE_SUFFIX)) as history_in_file:
        ...
        logging.info("Loaded last message times: {0}, {1}".format(last_2_message_in_time, last_1_message_in_time))
    ...
    if last_1_message_in_time is not None and last_2_message_in_time is not None:
        if (int((ut_now() - last_1_message_in_time)/60) - int((last_1_message_in_time - last_2_message_in_time)/60)) <= 2:
            ...
            logging.info("Similiar messages detected")
        else:
    ...
    for notify_item_name, notify_item in config_dict['notify_devilry']['notify'].items():
        ...
        if 'match' in notify_item:
            logging.info("Match filter enabled")
            ...
            for match_item_key, match_item_val in notify_item['match'].items():
                if message[match_item_key] != match_item_val:
                    ...
                    logging.info("Match filter matched all keys for notify item '{0}'".format(notify_item_name))
            if should_continue:
                logging.info("Match filter didn't match all keys, skipping notify item '{0}'".format(notify_item_name))
                continue
        else:
            logging.info("Match filter not enabled")
        ...
        for sending_method in SENDING_METHODS:
            ...
            if sending_method in notify_item:
                logging.info("Sending method '{0}' enabled for notify item {1}".format(sending_method, notify_item_name))
                ...
                for sending_method_item in notify_item[sending_method]:
                    logging.info("Contact alias '{0}' listed for current sending method".format(sending_method_item))
                    # Open history message in file
                    with open_history_message_file(HISTORY_DIR, "{0}_{1}_{2}_{3}_{4}.{5}".format(HISTORY_MESSAGE_OUT_PREFIX, key_combo, notify_item_name.replace(" ", "_"), sending_method.replace(" ", "_"), sending_method_item.replace(" ", "_"), HISTORY_MESSAGE_SUFFIX)) as history_out_file:
                        ...
                        if not sim_messages_detected:
                            ...
                            logging.info("Message out counter cleared out")
                        ...
                        logging.info("Last message time and message count for notify intem '{0}', sending method '{1}', contact alias '{2}': {3}, {4}".format(notify_item_name, sending_method, sending_method_item, last_message_out_time, message_out_count))
                        if last_message_out_time is None or message_out_count == 0:
                            logging.info("Last message time is None or messages count is 0, sending anyway")
                            should_send = True
                        else:
                            ...
                            if "rate_limit" in notify_item and sim_messages_detected:
                                logging.info("Rate limit enabled for notify item '{0}' and similiar messages detected".format(notify_item_name))
                                ...
                                logging.info("Took rate limit: {0}, time from last message: {1}".format(rate_limit_level, time_from_last_message))
                                if time_from_last_message >= rate_limit_level:
                                    # Send the message with this sending method and alias
                                    should_send = True
                                    logging.info("Time from last message is bigger than rate limit, should send")
                            # Else no rate limitng, always send
                            else:
                                should_send = True
                                logging.info("Rate limit not enabled for notify item '{0}' or similiar messages not detected".format(notify_item_name))
                        if should_send:
                            ...
                            try:
                                logging.info("Sending message for notify item '{0}' via '{1}' for contact alias '{2}'".format(notify_item_name, sending_method, sending_method_item))
                                ...
                                logging.warning("Caught exception on sending:")
                                logging.exception(e)
                        else:
                            logging.info("Rate limit applied, message not sent for notify intem '{0}', sending method '{1}', contact alias '{2}'".format(notify_item_name, sending_method, sending_method_item))
