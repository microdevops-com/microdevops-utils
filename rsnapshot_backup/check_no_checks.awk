BEGIN {
	...
	# Check File
	txt_file = "/opt/sysadmws/rsnapshot_backup/check_dot_backup.txt";
	i = 1;
	...
	# Fresh Files
	txt_file = "/opt/sysadmws/rsnapshot_backup/check_file_age.txt";
	i = 1;
	if (system("test -e " txt_file) == 0) {
	...
	# Mysql
	txt_file = "/opt/sysadmws/rsnapshot_backup/check_mysql.txt";
	i = 1;
	if (system("test -e " txt_file) == 0) {
	...
	# Postgresql
	txt_file = "/opt/sysadmws/rsnapshot_backup/check_postgresql.txt";
	i = 1;
	if (system("test -e " txt_file) == 0) {
	...
	ubuntu_backup_src_arr[8] = "/opt/sysadmws";
	#
	...
	debian_backup_src_arr[8] = "/opt/sysadmws";
	#
	...
	debian_backup_src_arr[7] = "/opt/sysadmws";
}
				...
				if ((check_file_array[i, 1] == host_name) || (check_file_array[i, 5] == host_name)) {
					if ((check_file_array[i, 2] == backup_src_arr[ii]) || (check_file_array[i, 6] == backup_src_arr[ii])) {
						if (index(check_file_array[i, 3], backup_dst) != 0) {
							if (show_notices == 1) {
								print_timestamp(); print("NOTICE: rsnapshot_backup found in check_dot_backup.txt: " host_name "	" backup_type "	" backup_src_arr[ii] "	" backup_dst " on line " FNR);
							}
							total_ok = total_ok + 1;
							backup_src_found = 1;
						}
				...
				if ((fresh_file_array[i, 1] == host_name) || (fresh_file_array[i, 10] == host_name)) {
					if ((fresh_file_array[i, 2] == backup_src_arr[ii]) || (fresh_file_array[i, 11] == backup_src_arr[ii])) {
						if (index(fresh_file_array[i, 3], backup_dst) != 0) {
							if (show_notices == 1) {
								print_timestamp(); print("NOTICE: rsnapshot_backup found in check_file_age.txt: " host_name "	" backup_type "	" backup_src_arr[ii] "	" backup_dst " on line " FNR);
							}
							total_ok = total_ok + 1;
							backup_src_found = 1;
						}
		...
		for (i = 1; i <= check_file_array_size; i++) {
			if (check_file_array[i, 1] == checked_host_name) {
				if ((check_file_array[i, 2] == backup_src) || (check_file_array[i, 6] == backup_src)) {
					if (index(check_file_array[i, 3], backup_dst) != 0) {
						if (show_notices == 1) {
							print_timestamp(); print("NOTICE: rsnapshot_backup found in check_dot_backup.txt: " host_name "	" backup_type "	" backup_src "	" backup_dst " on line " FNR);
						}
						total_ok = total_ok + 1;
						next;
					}
		...
		for (i = 1; i <= fresh_file_array_size; i++) {
			if (fresh_file_array[i, 1] == checked_host_name) {
				if (fresh_file_array[i, 2] == backup_src) {
					if (index(fresh_file_array[i, 3], backup_dst) != 0) {
						if (show_notices == 1) {
							print_timestamp(); print("NOTICE: rsnapshot_backup found in check_file_age.txt: " host_name "	" backup_type "	" backup_src "	" backup_dst " on line " FNR);
						}
						total_ok = total_ok + 1;
						next;
					}
			...
			if ((mysql_file_array[i, 1] == host_name) || (mysql_file_array[i, 5] == host_name)) {
				if ((mysql_file_array[i, 2] == backup_src) || ((mysql_file_array[i, 2] == "MYSQL_ALL") && (backup_src == "ALL"))) {
					if (index(mysql_file_array[i, 4], backup_dst) != 0) {
						if (show_notices == 1) {
							print_timestamp(); print("NOTICE: rsnapshot_backup found in check_mysql.txt: " host_name "	" backup_type "	" backup_src "	" backup_dst " on line " FNR);
						}
						total_ok = total_ok + 1;
						next;
					}
			...
			if ((pg_file_array[i, 1] == host_name) || (pg_file_array[i, 5] == host_name)) {
				if ((pg_file_array[i, 2] == backup_src) || ((pg_file_array[i, 2] == "POSTGRESQL_ALL") && (backup_src == "ALL"))) {
					if (index(pg_file_array[i, 4], backup_dst) != 0) {
						if (show_notices == 1) {
							print_timestamp(); print("NOTICE: rsnapshot_backup found in check_postgresql.txt: " host_name "	" backup_type "	" backup_src "	" backup_dst " on line " FNR);
						}
						total_ok = total_ok + 1;
						next;
					}
...
END {
	...
	if (total_errors == 0) {
		if (show_notices == 1) {
			print_timestamp(); print("NOTICE: Backup server " checked_host_name " rsnapshot_backup and check_backup configs compared OK: " total_ok);
		}
	} else {
