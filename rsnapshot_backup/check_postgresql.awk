...

{
	...
	host_name	= row_host;
	db_name		= row_source;
	dump_dir	= row_path "/.sync/rsnapshot/var/backups/postgresql";
...
END {
	...
	if (total_errors == 0) {
		if (show_notices == 1) {
			print_timestamp(); print("NOTICE: Backup server " checked_host_name " postgresql backups OK checks: " total_ok);
		}
	} else {
