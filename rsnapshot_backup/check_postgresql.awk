BEGIN {
	...
	total_errors	= 0;
	total_ok        = 0;
	check_backup_skip_postgresql_warning	= 0;
	# Get my hostname
...

{
        ...
        if (match($0, /^# check_backup_skip_postgresql_warning: True$/)) {
                check_backup_skip_postgresql_warning = 1;
                next;
        }
...
END {
        # Total lines check
        if ((total_lines == 0) && (check_backup_skip_postgresql_warning == 0))  {
                print_timestamp(); print("WARNING: Backup server " checked_host_name " postgresql backup check txt config empty");
        }
        if ((total_lines > 0) && (check_backup_skip_postgresql_warning == 1))  {
                print_timestamp(); print("WARNING: Backup server " checked_host_name " postgresql backup check txt config not empty but you have check_backup_skip_postgresql_warning: True set");
        }
	# Summ results
