...
SKIP_FILE=/opt/sysadmws/rsnapshot_backup/check_no_checks.skip
...
if [ -f $CONF_FILE ]; then
	awk -f /opt/sysadmws/rsnapshot_backup/check_no_checks.awk -v show_notices=$1 -v hostname_filter=$2 $CONF_FILE 2>&1
	exit $?
else
	date '+%F %T ' | tr -d '\n'
