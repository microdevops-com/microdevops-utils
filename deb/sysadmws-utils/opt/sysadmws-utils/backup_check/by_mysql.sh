...
if [ "$1" != "0" ] && [ "$1" != "1" ]; then
	...
	echo -e >&2 "ERROR: 0 to show only basic notices, 1 to show all notices, 2 to show basic notices"
...
CONF_FILE=/opt/sysadmws-utils/backup_check/by_mysql.txt
SKIP_FILE=/opt/sysadmws-utils/backup_check/by_mysql.skip
...
if [ -f $CONF_FILE ]; then
	awk -f /opt/sysadmws-utils/backup_check/by_mysql.awk -F '\t' -v show_notices=$1 -v hostname_filter=$2 $CONF_FILE 2>&1
	exit $?
else
	date '+%F %T ' | tr -d '\n'
