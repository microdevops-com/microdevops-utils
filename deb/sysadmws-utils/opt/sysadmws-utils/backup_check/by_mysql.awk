BEGIN {
	...
	total_errors	= 0;
	total_ok        = 0;
	backup_check_skip_mysql_warning	= 0;
	# Get my hostname
...

{
        ...
        if (match($0, /^# backup_check_skip_mysql_warning: True$/)) {
                backup_check_skip_mysql_warning = 1;
                next;
        }
...
END {
        # Total lines check
        if ((total_lines == 0) && (backup_check_skip_mysql_warning == 0))  {
                print_timestamp(); print("WARNING: Backup server " checked_host_name " mysql backup check txt config empty");
        }
        if ((total_lines > 0) && (backup_check_skip_mysql_warning == 1))  {
                print_timestamp(); print("WARNING: Backup server " checked_host_name " mysql backup check txt config not empty but you have backup_check_skip_mysql_warning: True set");
        }
	# Summ results
